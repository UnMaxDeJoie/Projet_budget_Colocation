...
class Config{
	...
	public const CNF = self::PROPERTIES; // .cnf
	...
	private function load(string $file, int $type = self::DETECT, array $default = []) : void{
		...
		if($this->type === self::DETECT){
			...
			$extension = strtolower($files[array_key_last($files)]);
	...
	public function save() : void{
		$content = match ($this->type) {
			...
			self::JSON => json_encode($this->config, $this->jsonOptions | JSON_THROW_ON_ERROR),
